...
<div class="max-w-screen-xl px-4 mx-auto sm:px-6 lg:px-8">
    ...
    <!-- Header dengan gradient TEAL (sama seperti dashboard) -->
                            ...
                            @if($unreadCount > 0)
                                <span class="inline-flex items-center gap-1">
                                    <span class="relative flex w-2 h-2">
                                        <span class="absolute inline-flex w-full h-full bg-teal-200 rounded-full opacity-75 animate-ping"></span>
                                        <span class="relative inline-flex w-2 h-2 bg-teal-300 rounded-full"></span>
                                    </span>
    ...
    <div class="overflow-hidden transition-all duration-300 transform bg-white border-0 shadow-xl rounded-3xl hover:shadow-2xl">
        <div class="px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-teal-50 via-emerald-50 to-teal-50">
            <div class="flex items-center justify-between">
                ...
                @if($notifications->total() > 0)
                    <div class="flex items-center gap-2 px-3 py-1.5 text-xs font-bold text-teal-700 bg-teal-100 border border-teal-200 rounded-full">
        ...
        <div class="divide-y divide-gray-100">
            @forelse($notifications as $notification)
                @php
                    ...
                    //  Warna sesuai dashboard (Teal, Blue, Purple, Pink)
                    $configs = [
                        'welcome' => [
                            'iconBg' => $isUnread ? 'bg-gradient-to-br from-teal-100 to-emerald-100 shadow-md' : 'bg-gray-100',
                            'iconColor' => $isUnread ? 'text-teal-600' : 'text-gray-400',
                            'bgColor' => $isUnread ? 'bg-gradient-to-r from-teal-50/60 to-emerald-50/40 border-l-4 border-teal-400' : 'bg-white',
                            ...
                            'buttonColor' => 'from-teal-600 to-emerald-700',
                        ],
                        'proker_assignment' => [
                            'iconBg' => $isUnread ? 'bg-gradient-to-br from-blue-100 to-indigo-100 shadow-md' : 'bg-gray-100',
                            'iconColor' => $isUnread ? 'text-blue-600' : 'text-gray-400',
                            'bgColor' => $isUnread ? 'bg-gradient-to-r from-blue-50/60 to-indigo-50/40 border-l-4 border-blue-400' : 'bg-white',
                        ...
                        'default' => [
                            'iconBg' => $isUnread ? 'bg-gradient-to-br from-teal-100 to-emerald-100 shadow-md' : 'bg-gray-100',
                            'iconColor' => $isUnread ? 'text-teal-600' : 'text-gray-400',
                            'bgColor' => $isUnread ? 'bg-gradient-to-r from-teal-50/60 to-emerald-50/40 border-l-4 border-teal-300' : 'bg-white',
                ...
                <div class="transition-all duration-300 hover:bg-gradient-to-r hover:from-teal-50/30 hover:to-emerald-50/20 {{ $config['bgColor'] }}">
                            ...
                            <div class="flex items-start justify-between gap-3 mb-2">
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        ...
                                        @if($isUnread)
                                            <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold text-teal-700 bg-teal-100 border border-teal-200 rounded-full shadow-sm animate-pulse">
                                                <span class="relative flex w-1.5 h-1.5">
                                                    <span class="absolute inline-flex w-full h-full bg-teal-400 rounded-full opacity-75 animate-ping"></span>
                                                    <span class="relative inline-flex w-1.5 h-1.5 bg-teal-500 rounded-full"></span>
                                                </span>
                                                Baru
                                            </span>
                                        @endif
